...
[分析流程——必须严格执行]
步骤1：阅读每个分类的标题和说明（prompt），在心里明确每个分类的关注角度是什么。
步骤2：对每个分类，单独判断：这份网页与该分类有关吗？只看该分类的说明，不要想其他分类。有关则 relevance >= 50，无关则跳过。
步骤3：为每个相关分类，从头写一套全新的 findings / notes / keyQuotes。写的时候只关注该分类的角度，完全忘掉其他分类。两套分析之间不允许共享任何 finding、note 或 quote。
步骤4：输出前自检——如果某条 finding 或 note 贴到另一个 match 里也成立，说明你没有做到独立分析，必须重写。

[输出格式]
只输出 JSON（不要输出任何其他内容）：
{"summary":"80字以内页面摘要","keywords":["关键词"],"matches":[{"goalId":"g_xxx","taskId":"t_xxx或null","subtaskId":"s_xxx或null","relevance":0,"reasoning":"为什么与该分类相关","findings":["关键发现1","关键发现2"],"notes":[{"topic":"主题","content":"详细笔记"}],"keyQuotes":[{"quote":"原文关键句","context":"上下文"}]}]}
...
4. 【最关键】每个 match 的 findings / notes / keyQuotes 必须完全不同。不同分类的关注角度由各自的"说明"（prompt）定义，你应从该分类的视角审视网页。
5. 判断"什么是有价值的信息"时，以该分类的说明为准。分类说明里写了关注什么主题、什么关键词——你就据此提取。不要写一个"通用版"然后复制给多个分类。
6. findings 写该分类视角下的关键信息点。允许详细展开，1 条 finding 可以写 1-4 句话。信息量大的页面可以写到 8-10 条。禁止写"在不同领域有应用""具有重要价值"这类空话。
7. notes 按主题拆分，每个主题一条。content 先引用原文关键句（用引号），紧接着写分析——这个信息在该分类视角下为什么有价值。允许详细展开，3-10 句话都可以。
8. keyQuotes 尽量多提供，最多 6 条。quote 必须逐字来自网页正文，不得改写。
9. reasoning 必须引用网页里的具体内容说明为什么与该分类相关。
10. 内容丰富的网页，请充分利用输出空间，不要刻意精简。
11. 拿不准是否相关时，宁可判定为不相关。

[错误示例——绝对禁止]
下面的输出是错误的，因为两个 match 的分析内容完全一样，只是把 goalId 换了：
{"matches":[
  {"goalId":"g1","taskId":null,"subtaskId":null,"relevance":85,"reasoning":"页面包含skill相关内容","findings":["页面讨论了skill的运营和推广"],"notes":[{"topic":"skill运营","content":"页面包含skill运营相关信息"}]},
  {"goalId":"g2","taskId":null,"subtaskId":null,"relevance":80,"reasoning":"页面包含skill相关内容","findings":["页面讨论了skill的运营和推广"],"notes":[{"topic":"skill运营","content":"页面包含skill运营相关信息"}]}
]}
